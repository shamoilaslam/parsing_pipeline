...
def _select_documents(
    records: list[dict[str, Any]], target: int, seed: int, coverage_min: int = 3,
    true_mode_targets: dict[str, float] | None = None,
) -> list[dict[str, Any]]:
    """Two-phase selection:
    ...
    for key, group in mode_urdu.items():
        mode_key, _urdu_key = key
        if true_mode_targets and mode_key in true_mode_targets:
            # true mode share x this pool's own urdu-within-mode ratio -
            # best available estimate when we don't have a true urdu total.
            mode_pool_total = sum(len(g) for k, g in mode_urdu.items() if k[0] == mode_key)
            urdu_share_within_mode = len(group) / mode_pool_total if mode_pool_total else 0
            cell_target = round(true_mode_targets[mode_key] * urdu_share_within_mode * target)
        else:
...
def build_manifest(
    ...
    if from_manifest:
        # Re-select from an already-selected pool (e.g. an earlier 250-doc
        # run) instead of scanning the whole corpus again. Only re-profiles
        # the documents already in that manifest - fast, since it's however
        # many docs are in there, not the full corpus - but re-profiles them
        # with the current (fixed) detection logic rather than trusting
        # whatever an older run computed.
        old = json.loads(from_manifest.read_text(encoding="utf-8"))
        pdfs = [Path(d["path"]) for d in old["documents"]]
        # If the earlier run recorded true corpus-wide mode counts, use
        # those as the proportional target instead of this pool's own mix -
        # the pool itself may already be hardness-skewed relative to the
        # full corpus, and skewed-in skewed-out isn't what "proportional"
        # is supposed to mean.
        old_modes = old.get("corpus_summary", {}).get("modes")
        if old_modes and sum(old_modes.values()) > 0:
            total_old = sum(old_modes.values())
            true_mode_targets = {k: v / total_old for k, v in old_modes.items()}
            print(f"  using true corpus-wide mode proportions from {from_manifest}: {old_modes}", file=sys.stderr)
        root = root or Path(old.get("sampling", {}).get("root", "."))
    else:
        if root is None:
            raise SystemExit("either ROOT or --from-manifest is required")
        pdfs = sorted(_iter_pdfs(root))

    if not pdfs:
        raise SystemExit(f"No PDFs found (root={root}, from_manifest={from_manifest})")
    ...
    selected = _select_documents(records, min(target_docs, len(records)), seed, coverage_min, true_mode_targets)
...
def main() -> None:
    ...
    parser.add_argument("root", type=Path, nargs="?", default=None, help="Corpus root (not needed if using --from-manifest)")
    ...
    parser.add_argument("--from-manifest", type=Path, default=None, help="re-select from an already-selected pool (e.g. an earlier 250-doc manifest) instead of scanning the whole corpus - only re-profiles those documents, with current fixes applied")
    ...
    report = build_manifest(
        ...
        args.coverage_min, args.from_cache, args.from_manifest,
    )
